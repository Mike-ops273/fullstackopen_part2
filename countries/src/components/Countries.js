...
const Countries = ({ countries, query }) => {
  ...
  if (query == "") {
    return <>{""}</>;
  } else if (queryCountries.length > 10) { //if too many results
    return <>Too many countries, please be more specific</>;
  } else if (queryCountries.length <= 10 && queryCountries.length > 1) { //if 10 results or less
    console.log("in here");
  ...
  } else if (queryCountries.length == 1) { //a singular result
    return (
      <>
